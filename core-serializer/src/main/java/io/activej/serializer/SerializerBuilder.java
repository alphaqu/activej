...
import io.activej.codegen.ClassBuilder.BytecodeOutput;
...
public final class SerializerBuilder {
	...
	private static final BytecodeOutput DEFAULT_BYTE_CODE_OUTPUT = DEFAULT_SAVE_DIR == null ?
			BytecodeOutput.none() :
	...
	private BytecodeOutput byteCodeOutput = DEFAULT_BYTE_CODE_OUTPUT;
	...
	public SerializerBuilder withGeneratedBytecodePath(Path path) {
		this.byteCodeOutput = BytecodeOutput.toDirectory(path);
		return this;
	}

	/**
	 * Allows to send generated bytecode to some {@link java.io.OutputStream}
	 *
	 * @param byteCodeOutput defines an output stream for each generated class
	 */
	public SerializerBuilder withBytecodeOutput(BytecodeOutput byteCodeOutput) {
	...
	public <T> BinarySerializer<T> build(SerializerDef serializer) {
		...
		ClassBuilder<BinarySerializer> classBuilder = ClassBuilder.create(classLoader, BinarySerializer.class)
				.withClassKey(classKey)
				.withBytecodeOutput(byteCodeOutput);
		if (className != null) {
